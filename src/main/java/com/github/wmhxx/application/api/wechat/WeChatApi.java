...
import static com.github.wmhxx.application.api.wechat.WeChatClient.LOG_TAG;
...
final class WeChatApi {
    ...
    RspLogin login() {
        ...
        XHttpExecutor.Response http = XTools.http(httpExecutor, request);
        XTools.logD(LOG_TAG, "监听登录");
        RspLogin rspLogin = new RspLogin(http.string());
     ...
     * @param url 登录url
     */
    void webWeChatNewLoginPage(String url) {
        HashMap<String, String> headersMap = new HashMap<>();
        headersMap.put("client-version", "2.0.0");
        ...
        String rspStr = HttpUtil.createGet(url).addHeaders(headersMap).execute().body();
        System.out.println("【rspStr】 | rspStr | " + rspStr);
    ...
    RspInit webWeChatInit() {
        ...
        request.content(new XRequest.StringContent(XRequest.MIME_JSON, JSONUtil.toJsonStr(new ReqInit(new BaseRequest(uin, sid, skey)))));
        RspInit rspInit = JSONUtil.toBean(XTools.http(httpExecutor, request).string(), RspInit.class);
     ...
     * @return 接口调用结果
     */
    RspStatusNotify webWeChatStatusNotify(String userName, int notifyCode) {
        XRequest request = XRequest.POST(String.format("https://%s/cgi-bin/mmwebwx-bin/webwxstatusnotify", host));
        request.query("pass_ticket", this.passticket);
        request.content(new XRequest.StringContent(XRequest.MIME_JSON, JSONUtil.toJsonStr(new ReqStatusNotify(new BaseRequest(uin, sid, skey), notifyCode, userName))));
        return JSONUtil.toBean(XTools.http(httpExecutor, request).string(), RspStatusNotify.class);
    }
     ...
     * @return 联系人列表
     */
    RspGetContact webWechatGetContact() {
        ...
        request.query("pass_ticket", this.passticket);
        return JSONUtil.toBean(XTools.http(httpExecutor, request).string(), RspGetContact.class);
    }
     ...
     * @return 联系人的详细信息
     */
    RspBatchGetContact webWeChatBatchGetContact(List<ReqBatchGetContact.Contact> contactList) {
        XRequest request = XRequest.POST(String.format("https://%s/cgi-bin/mmwebwx-bin/webwxbatchgetcontact", host));
        request.query("r", System.currentTimeMillis());
        request.query("type", "ex");
        request.query("pass_ticket", this.passticket);
        request.content(new XRequest.StringContent(XRequest.MIME_JSON, JSONUtil.toJsonStr(new ReqBatchGetContact(new BaseRequest(uin, sid, skey), contactList))));
        return JSONUtil.toBean(XTools.http(httpExecutor, request).string(), RspBatchGetContact.class);
    }
    ...
    RspSyncCheck synccheck() {
        RspInit.SyncKey syncKeys = this.syncCheckKey != null ? this.syncCheckKey : this.synckey;
        ...
        String body = HttpUtil.createGet("https://webpush.wx.qq.com/cgi-bin/mmwebwx-bin/synccheck?" + param)
                .execute().body();
        return new RspSyncCheck(body);
    }
    ...
    RspSync webwxsync() {
        ...
        request.query("skey", this.skey);
        request.query("pass_ticket", this.passticket);
        request.content(new XRequest.StringContent(XRequest.MIME_JSON, JSONUtil.toJsonStr(new ReqSync(new BaseRequest(uin, sid, skey), this.synckey))));
        RspSync rspSync = JSONUtil.toBean(XTools.http(httpExecutor, request).string(), RspSync.class);
     ...
     * @return 发送的结果
     */
    RspSendMsg webWeChatSendMsg(ReqSendMsg.Msg msg) {
        XRequest request = XRequest.POST(String.format("https://%s/cgi-bin/mmwebwx-bin/webwxsendmsg", host));
        request.query("pass_ticket", this.passticket);
        request.content(new XRequest.StringContent(XRequest.MIME_JSON, JSONUtil.toJsonStr(new ReqSendMsg(new BaseRequest(uin, sid, skey), msg))));
        return JSONUtil.toBean(XTools.http(httpExecutor, request).string(), RspSendMsg.class);
    }
     ...
     * @return 撤回结果
     */
    RspRevokeMsg webWeChatRevokeMsg(long clientMsgId, long serverMsgId, String userName) {
        XRequest request = XRequest.POST(String.format("https://%s/cgi-bin/mmwebwx-bin/webwxrevokemsg", host));
        request.content(new XRequest.StringContent(XRequest.MIME_JSON, JSONUtil.toJsonStr(new ReqRevokeMsg(new BaseRequest(uin, sid, skey), String.valueOf(clientMsgId), String.valueOf(serverMsgId), userName))));
        return JSONUtil.toBean(XTools.http(httpExecutor, request).string(), RspRevokeMsg.class);
    }
     ...
     * @return 获取到的图片文件
     */
    File webWeChatGetMsgImg(long msgId, String type) {
     ...
     * @return 获取到的语音文件
     */
    File webWeChatGetVoice(long msgId) {
     ...
     * @return 获取到的视频文件
     */
    File webWeChatGetVideo(long msgId) {
     ...
     * @return 获取到的附件文件
     */
    File webWeChatGetMedia(long msgId, String filename, String mediaId, String sender) {
     ...
     * @return 发送的结果
     */
    RspVerifyUser webWeChatVerifyUser(int opCode, String userName, String verifyTicket, String verifyContent) {
        XRequest request = XRequest.POST(String.format("https://%s/cgi-bin/mmwebwx-bin/webwxverifyuser", host));
        request.query("r", System.currentTimeMillis());
        request.query("pass_ticket", this.passticket);
        request.content(new XRequest.StringContent(XRequest.MIME_JSON, JSONUtil.toJsonStr(new ReqVerifyUser(new BaseRequest(uin, sid, skey), opCode, userName, verifyTicket, verifyContent))));
        return JSONUtil.toBean(XTools.http(httpExecutor, request).string(), RspVerifyUser.class);
    }
     ...
     * @return 修改备注的结果
     */
    RspOplog webWeChatOplog(int cmdId, int op, String userName, String remarkName) {
        XRequest request = XRequest.POST(String.format("https://%s/cgi-bin/mmwebwx-bin/webwxoplog", host));
        request.query("pass_ticket", this.passticket);
        request.content(new XRequest.StringContent(XRequest.MIME_JSON, JSONUtil.toJsonStr(new ReqOplog(new BaseRequest(uin, sid, skey), cmdId, op, userName, remarkName))));
        return JSONUtil.toBean(XTools.http(httpExecutor, request).string(), RspOplog.class);
    }
     ...
     * @return 添加或移除的结果
     */
    RspUpdateChatroom webWeChatUpdateChartRoom(String chatroom, String fun, String name, List<String> memberList) {
        XRequest request = XRequest.POST(String.format("https://%s/cgi-bin/mmwebwx-bin/webwxupdatechatroom", host));
        request.query("fun", fun);
        request.query("pass_ticket", this.passticket);
        request.content(new XRequest.StringContent(XRequest.MIME_JSON, JSONUtil.toJsonStr(new ReqUpdateChatroom(new BaseRequest(uin, sid, skey), chatroom, fun, name, XTools.strJoin(memberList, ",")))));
        return JSONUtil.toBean(XTools.http(httpExecutor, request).string(), RspUpdateChatroom.class);
    }
     ...
     * @return 秒传结果，
     */
    RspCheckUpload webWeChatCheckUpload(File file, String fromUserName, String toUserName) {
        XRequest request = XRequest.POST(String.format("https://%s/cgi-bin/mmwebwx-bin/webwxcheckupload", host));
        request.content(new XRequest.StringContent(XRequest.MIME_JSON, JSONUtil.toJsonStr(new ReqCheckUpload(new BaseRequest(uin, sid, skey), file, fromUserName, toUserName))));
        return JSONUtil.toBean(XTools.http(httpExecutor, request).string(), RspCheckUpload.class);
    }
     ...
     */
    RspUploadMedia webWeChatUploadMedia(String fromUserName, String toUserName, File file, String aesKey, String signature) throws IOException {
        ...
        if (file.length() < 1024L * 1024L) {
            ...
            request.content("uploadmediarequest", JSONUtil.toJsonStr(new ReqUploadMedia(new BaseRequest(uin, sid, skey, deviceId), clientMediaId, 2, fileLength, 0, fileLength, fileMd5, aesKey, signature, fromUserName, toUserName)));
            ...
            return JSONUtil.toBean(XTools.http(httpExecutor, request).string(), RspUploadMedia.class);
        } else {
            ...
            try (BufferedInputStream bfinStream = new BufferedInputStream(new FileInputStream(file))) {
                for (long sliceIndex = 0, sliceCount = (long) Math.ceil(file.length() / 512D / 1024D); sliceIndex < sliceCount; sliceIndex++) {
                    ...
                    rspUploadMedia = JSONUtil.toBean(XTools.http(httpExecutor, request).string(), RspUploadMedia.class);
                }
            }
